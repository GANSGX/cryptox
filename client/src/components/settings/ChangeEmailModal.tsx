...
export function ChangeEmailModal({
  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await apiService.verifyPasswordAndSendCode(
        user!.username,
        password,
      );
  ...
  const handleCodeSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await apiService.verifyCurrentEmailCode(
        user!.username,
        code,
      );
  ...
  return createPortal(
    <>
      ...
      <div className={`change-email-modal ${isAnimated ? "open" : ""}`}>
        ...
        {/* Content */}
        <div className="change-email-content">{renderStep()}</div>
      </div>
    </>,
    document.body,
  );
