...
### 3. Stress Test (💥 Breaking Point) 🔥 EXTREME

**Цель:** НАЙТИ АБСОЛЮТНЫЙ ПРЕДЕЛ! Нагружаем до 15,000 users

```bash
k6 run k6/stress-test.js --env BASE_URL=http://localhost:3000
```

**Параметры (Pentagon-level):**

- 0 → 1,000 → 2,500 → 5,000 → 10,000 → 15,000 VU
- 16 минут
- Ожидается: Найти breaking point при 10,000+ users, допустимо до 50% ошибок

**Цель:** Узнать АБСОЛЮТНЫЙ ПРЕДЕЛ системы при экстремальной нагрузке

---

### 4. Spike Test (⚡ DDoS Simulation) 🔥 EXTREME

**Цель:** Симуляция MASSIVE DDoS атаки - 50,000 users!

```bash
k6 run k6/spike-test.js --env BASE_URL=http://localhost:3000
```

**Параметры (DDoS Attack Simulation):**

- 100 → **10,000 VU (5 sec)** → **50,000 VU (10 sec)!** → 100
- 5.5 минут
- Проверяет: Rate limiting, system stability, recovery
- Допустимо до 70% ошибок (rate limiting блокирует атаку)
